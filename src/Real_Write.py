...
class WriteRealisation():
    ...
    def get_income_loss(self):
        ...
        # Income
        for row in range(self.mks.B_IN_ST_LINE,self.mks.B_DETAIL_LINE_ST - 3):
        ...
        # Loss
        for row in range(self.mks.B_LOS_ST_LINE,self.mks.B_DETAIL_LINE_ST - 3):
    ...
    def clear_real(self):
        clear_zone(self.ws,self.mks.B_IN_ST_LINE,self.mks.B_DETAIL_LINE_ST - 3,self.mks.B_IN_NAME_COL,self.mks.B_IN_RE_COL)
        clear_zone(self.ws,self.mks.B_LOS_ST_LINE,self.mks.B_DETAIL_LINE_ST - 3,self.mks.B_LOS_NAME_COL,self.mks.B_LOS_RE_COL)
        ...
        while ((self.mks.B_DETAIL_LINE_ST > (self.mks.B_IN_ST_LINE + REAL_SPACE_INC_LOSS_2_DETAIL + 3)) and 
                (self.mks.B_DETAIL_LINE_ST > (self.mks.B_LOS_ST_LINE + REAL_SPACE_INC_LOSS_2_DETAIL + 3))):
    ...
    def print_tot(self):
        ...
        self.ws.cell(r1,c1+3).border   = Border(left=Side(style='thin'),right=Side(style='thin'),top=Side(style='thick'),bottom=Side(style='thin'))
    ...
    def print_real_income(self):
        # Add the real incom in the incom tab (add a new line if the expected does not exist)
    ...
    def print_real_loss(self):
        ...
        for loss in self.los_list:
            if check_cell_value(loss.name)==0 and(check_cell_value(loss.ex) in [1,2] or check_cell_value(loss.re) in [1,2]):
                while (self.mks.B_DETAIL_LINE_ST <= (self.mks.B_los_ed_line + REAL_SPACE_INC_LOSS_2_DETAIL + 2)):
                ...
                self.ws.merge_cells(start_row=self.mks.B_los_ed_line, start_column=self.mks.B_LOS_NAME_COL,
                                    end_row=self.mks.B_los_ed_line, end_column=self.mks.B_LOS_NAME_COL+1)
    ...
    def set_protection(self):
        unprotect_range(self.ws,self.mks.B_IN_ST_LINE,self.mks.B_DETAIL_LINE_ST-3,self.mks.B_IN_NAME_COL,self.mks.B_REAL_COL_ED)
        unprotect_range(self.ws,self.mks.B_LOS_ST_LINE,self.mks.B_DETAIL_LINE_ST-3,self.mks.B_LOS_NAME_COL,self.mks.B_REAL_COL_ED)