...
class WriteExtraction():
    ...
    def supress_old(self):
        ...
        for row in range(self.mks.B_LOS_IN_ST_LINE,self.mks.B_in_ed_line+1):
        ...
        for row in range(self.mks.B_LOS_IN_ST_LINE,self.mks.B_los_ed_line+1):
    ...
    def up_mrks(self):
        ...
        self.mks.b_ext_verif = [self.mks.b_ext_sold_ed_est[0],self.mks.b_ext_verif[1]]
    ...
    def write_income_loss(self):
        self.ws.cell(row=self.mks.B_IN_TOT_EX[0],column=self.mks.B_IN_TOT_EX[1]).value = f"=SUM({get_column_letter(self.mks.B_IN_EX_COL)}{self.mks.B_LOS_IN_ST_LINE}:{get_column_letter(self.mks.B_IN_EX_COL)}{self.mks.B_in_ed_line})"
        self.ws.cell(row=self.mks.B_LOS_TOT_EX[0],column=self.mks.B_LOS_TOT_EX[1]).value = f"=SUM({get_column_letter(self.mks.B_LOS_EX_COL)}{self.mks.B_LOS_IN_ST_LINE}:{get_column_letter(self.mks.B_LOS_EX_COL)}{self.mks.B_los_ed_line})"
