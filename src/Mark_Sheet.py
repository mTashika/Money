    ...
    def extract_balise(self):
        ...
        for row in range(self.B_IN_ST_LINE, self.B_DETAIL_LINE_ST):
            ...
            if check_cell_value(cv) != 0:
                ...
                if self.B_in_ed_line<self.B_IN_ST_LINE:
                    self.B_in_ed_line=self.B_IN_ST_LINE
                break
            
        for row in range(self.B_LOS_ST_LINE, self.B_DETAIL_LINE_ST):
            cv = self.sheet.cell(row=row, column=self.B_LOS_NAME_COL).value
            if check_cell_value(cv) != 0:
                self.B_los_ed_line = row-1
                if self.B_los_ed_line<self.B_LOS_ST_LINE:
                    self.B_los_ed_line=self.B_LOS_ST_LINE
                break
